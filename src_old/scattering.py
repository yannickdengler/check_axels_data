...
def result_sampled(beta,m0,N_L,E_pipi,E_pipi_em,E_pipi_ep,dvec,mpi,num_gaussian=500):
    res = get_rizz(E_pipi,N_L,dvec,mpi)
    res_sample = {}
    for key in res.keys():
        res_sample[key] = []
...
def pstar(Ecm):
    return np.sqrt(Ecm/2-1)

def q_pstar(pstar, N_L):
    return pstar*L/(2*np.pi)
...
def calc_PS():
    ...
    res, res_sampled = result_sampled(6.9, -0.92, NL_arr, en_arr, en_m_arr, en_p_arr, dvec_arr, mpi)
