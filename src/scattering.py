...
def get_rizz(E_pipis, N_Ls, dvecs, mpi):
    ...
    for i in range(len(E_pipis)):
        ...
        if En_prime**2 - P_prime**2 < 4:
            ...
            result["pstar"].append(np.sqrt(result["s_prime"][i]/4-1))
            result["p2star"].append(result["pstar"][i]**2)
            result["pstar_prime"].append(result["pstar"][i]/mpi)
            result["p2star_prime"].append(result["p2star"][i]/mpi**2)
